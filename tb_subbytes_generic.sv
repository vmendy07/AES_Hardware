...
module tb_subbytes_generic;
  ...
  //--------------------------------------------------------------------------
  // Golden Model Function for SubBytes Transformation.
  //--------------------------------------------------------------------------
  function [127:0] subbytes_golden_model;
    input [127:0] state;
    input logic mode;
    reg [7:0] sbox[0:255];
    ...
    integer i;
    begin
      // Initialize the S-box and inverse S-box
      sbox = '{
      ...
      // Perform the substitution using the selected S-box
      for (i = 0; i < 16; i = i + 1) begin
        result[i*8 +: 8] = mode ? inv_sbox[state[i*8 +: 8]] : sbox[state[i*8 +: 8]];
      end
  ...
  // with the expected value generated by subbytes_golden_model.
  //--------------------------------------------------------------------------
  task run_test;
    ...
    begin
      $display("\n********** Test %0d **********", test_num);
      state = test_vector;
      expected_state = subbytes_golden_model(test_vector, mode);
      @(posedge clk); // Wait for a clock edge
      if (state_out === expected_state)
         $display("Test %0d PASS: Expected = %h, Received = %h", test_num, expected_state, state_out);
      else begin
         $display("Test %0d FAILED: Expected = %h, Received = %h", test_num, expected_state, state_out);
         all_tests_passed = 0;
      end
    end
  endtask
  ...
  initial begin
    ...
    // Forward SubBytes Tests
    mode = 0; // Forward mode
    run_test(128'h00000000000000000000000000000000, 1);
    run_test(128'hFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF, 2);
    run_test(128'h00102030405060708090A0B0C0D0E0F0, 3);
    run_test(128'hAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA, 4);
    run_test(128'h000102030405060708090A0B0C0D0E0F, 5);

    // Inverse SubBytes Tests
    mode = 1; // Inverse mode
    run_test(128'h63636363636363636363636363636363, 6);
    run_test(128'h16161616161616161616161616161616, 7);
    run_test(128'hE0E0E0E0E0E0E0E0E0E0E0E0E0E0E0E0, 8);
    run_test(128'h63CAB7040953D051CD60E0E7BA70E18C, 9);

    // Random test vector
    rand_vector = { $urandom, $urandom, $urandom, $urandom };
    run_test(rand_vector, 10);

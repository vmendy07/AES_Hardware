...
module tb_key_expansion;
    ...
    reg [127:0] seed_key;
    reg encrypt;  // New signal to select encryption or decryption mode
    ...
    key_expansion #(4, 10) key_expansion_instance (
        .encrypt(encrypt),  // Connect the encrypt signal
        .seed_key(seed_key),
    ...
    task display_expanded_key;
        ...
        begin
            $display("------------------------------------------------------");
            $display("Mode: %s", encrypt ? "Encryption" : "Decryption");  // Display mode
    ...
    initial begin
        ...
        // Test Case 2: Standard 128-bit AES key for decryption
        encrypt = 0;  // Set to decryption mode
        seed_key = 128'h2b7e151628aed2a6abf7158809cf4f3c;  
        #10;  
        display_expanded_key;

        // Test Case 3: Another 128-bit AES key for encryption
        encrypt = 1;  // Set to encryption mode
        seed_key = 128'h00112233445566778899aabbccddeeff;  
        #10;  
        display_expanded_key;

        // Test Case 4: Another 128-bit AES key for decryption
        encrypt = 0;  // Set to decryption mode
        seed_key = 128'h00112233445566778899aabbccddeeff;  
        #10;  
        display_expanded_key;

        // Test Case 5: Edge case - all bits set to 0
        seed_key = 128'h0;  
        #10;  
        display_expanded_key;

        // Test Case 6: Edge case - all bits set to 1
        seed_key = 128'hffffffffffffffffffffffffffffffff;  
        #10;  
        display_expanded_key;

        // Test Case 7: Random 128-bit AES key
        seed_key = 128'h1a2b3c4d5e6f708192a2b3c4d5e6f708;  
        #10;  
        display_expanded_key;

        // Test Case 8: Another random key
        seed_key = 128'h1234567890abcdef1234567890abcdef;  
        #10;  
        display_expanded_key;
        
        // Test Case 9: Another random key
